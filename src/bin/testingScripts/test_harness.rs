...
#[derive(Parser, Debug)]
#[command(author, version, about = "zk-DEAP Test Harness")]
...
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
enum TestFunction { DKG1, DKG2, DKG3, PartialGen, PartialVerify, AggCompute, ProofGen, ProofVerify, BaselineRound } //Function being tested (shared + ZKP-specific).
...
#[allow(dead_code)]
struct DeviceOutput { device_id: u32, key_package: Vec<u8>, public_package: Vec<u8>, signing_pubkey: [u8; 32], signing_seed: [u8; 32] } //DKG ceremony output for one device.
...
struct TestRunner { fixture_client: FixtureClient, output_file: File, vm_profile: String, device_id: u32, halo2_setup: Option<snark::Halo2Setup>, peer_keys_cache: HashMap<(usize, usize), HashMap<u32, VerifyingKey>>, last_components: Option<ProofComponents> } //Main test runner with caching.
impl TestRunner {
    ...
    fn run_all_tests(&mut self) { //Run all tests with optimized structure (shared functions once, ZKP-specific per type)
        ...
        let network_scaled_functions = vec![TestFunction::PartialGen, TestFunction::AggCompute, TestFunction::DKG1, TestFunction::DKG2, TestFunction::DKG3, TestFunction::BaselineRound]; // Functions that scale with network.
    ...
    fn run_single_test(&mut self, config: &TestConfig) -> Result<Metrics, Box<dyn std::error::Error>> { //Run a single test with full instrumentation
        ...
        let cpu_after = Self::get_cpu_times().unwrap_or((0, 0)); let (user_time_us, system_time_us) = (cpu_after.0.saturating_sub(cpu_before.0), cpu_after.1.saturating_sub(cpu_before.1));
    ...
    fn run_function(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //Route to the appropriate test function.
    ...
    fn test_dkg_phase1(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { let t0 = Instant::now(); let (_secret_pkg, round1_pkg) = common::dkg_phase1(config.device_id, config.n, config.t)?; let us = t0.elapsed().as_micros(); let size = bincode::serialize(&round1_pkg)?.len(); Ok((us, size)) } //DKG Phase 1 (uses common module directly)
    fn test_dkg_phase2(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //DKG Phase 2 (uses common module directly)
        ...
        let t0 = Instant::now(); let (_round2_secret, round2_packages) = common::dkg_phase2(secret_pkg, &received_r1)?; let us = t0.elapsed().as_micros(); //MEASURED OPERATION
    ...
    fn test_dkg_phase3(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //DKG Phase 3 (uses common module directly)
        ...
        let t0 = Instant::now(); let (key_package, public_package) = common::dkg_phase3(&round2_secret, &received_r1, &received_r2)?; let us = t0.elapsed().as_micros(); //MEASURED OPERATION
    ...
    fn test_partial_generation(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //Partial Generation (uses common module directly)
        ...
        let t0 = Instant::now(); let partial = common::generate_partial_decryption(config.device_id, &key_package, &public_package, &signing_key, &ciphertexts)?; let us = t0.elapsed().as_micros(); //MEASURED OPERATION
    ...
    fn test_partial_verification(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //Partial Verification (uses common module directly)
        ...
        let t0 = Instant::now(); let verified = common::receive_partial(partial, &peer_keys, &public_package, &ciphertexts, &mut verified_partials, &mut rates)?; let us = t0.elapsed().as_micros(); //MEASURED OPERATION
    ...
    fn test_aggregate_computation(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //Aggregate Computation (uses common module directly)
        ...
        let t0 = Instant::now(); let (count, total) = common::compute_aggregate(config.t, &ciphertexts, &verified_partials)?; let us = t0.elapsed().as_micros(); //MEASURED OPERATION
    ...
    fn test_proof_verification(&mut self, config: &TestConfig) -> Result<(u128, usize), Box<dyn std::error::Error>> { //Proof Verification (ZKP-specific)
        ...
        let public_package = frost::keys::PublicKeyPackage::deserialize(&device_output.public_package).map_err(|e| format!("PublicPackage deser: {:?}", e))?;
        let peer_keys = self.get_cached_peer_keys(config.n, config.t)?; //Get peer keys
        let t0 = Instant::now(); //MEASURED OPERATION starts here
