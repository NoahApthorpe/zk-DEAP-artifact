...
fn threshold_ratios() -> Vec<f64> {
    match std::env::var("ZKDEAP_RATIOS") { Ok(v) if !v.trim().is_empty() => v.split(',').filter_map(|x| x.trim().parse().ok()).collect(), _ => THRESHOLD_RATIOS.to_vec() }
}
